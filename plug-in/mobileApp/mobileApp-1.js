...
let mode = "all";
let filterList = [];
let horizontalline = document.getElementById("underline");
let horizontalBars = document.querySelectorAll(".task-tabs div");
let taskList = []
addButton.addEventListener("click",addTask)

horizontalBars.forEach(menu=>
  ...
  for (let i=0;i<list.length;i++){
    if (list[i].isComplete == true){
      ...
      <div>
        ...
        <button onclick="deleteTask('${list[i].id}')"><i class="fas fa-square-minus"></i></button>
      </div>
    </div>`;
    } else {
      resultHTML += `<div class="task"> 
